...
$string['error_existing_email'] = 'A user account with this email already exists at UKfilmNet. Make sure you are using your own school email!';
...
$string['error_missing_total_courses'] = 'Missing courses requested';
$string['error_total_courses_excessive'] = 'The max number allowed here is 5';
...
$string['setup_script_run_confirmation'] = '<div style="padding:20px"><p>Dear Moodle Admin,</p><p>You have run the UKFN Enrol Plugin setup script. Don\'t forget that there are also several setup tasks that you must complete manually. A document entitled "UKfilmNet Moodle enrolment plugin special settings" describes what the setup script does programmatically, and what must be done manually by the site administrator in order to make sure the site is set up to work properly. A copy of the document should be found in the plugin code at /enrol/ukfilmnet/assets/UKfilmNet_Moodle_enrolment_plugin_special_settings.pdf.</p><p><strong>Important!!</strong> Before you do anything else, please check to see whether any of the Setup Script Results in the list above suggest that you should run the Setup Script again. If they do, please run the Setup Script again by refreshing this page. Due to script dependencies, it is possible that the Setup Script will need to be run multiple times in order to create the "UKfilmNet Applicant Info" profile category which is necessary for this plugin to work properly.</p><p>When you have finished reading the above and the Setup Script Results list no longer suggests that you run it again, you may click on the browser\'s back arrow button to return to the page you were on before you ran the setup script.</p></div>';
